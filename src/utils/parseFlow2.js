...
export function parseGSCChartWorkbook(wb) {
  try {
    ...
    if (wb.SheetNames.includes('Filters')) {
      const filterRows = XLSX.utils.sheet_to_json(wb.Sheets['Filters'], { header: 1, defval: '', raw: true })
      for (const row of filterRows) {
        const key = String(row[0] ?? '').trim().toLowerCase()
        const val = String(row[1] ?? '').trim()
        if (key === 'page') {
          if (val.includes('/dijual/')) { segment = 'dijual'; break }
          if (val.includes('/disewa/')) { segment = 'disewa'; break }
          if (val.includes('/articles-all/')) { segment = 'blog'; break }
        }
      }
    }
    ...
    const chartRows = XLSX.utils.sheet_to_json(wb.Sheets['Chart'], { header: 1, defval: '', raw: true })
    ...
    for (let i = 1; i < chartRows.length; i++) {
      ...
      const pos = toNum(r[4])
      if (pos > 0) { positionSum += pos; positionCount++ }
    }
...
export function parseGA4FreeFile(csvText) {
  ...
  for (let i = 8; i < lines.length; i++) {
    ...
    const v = toNum(r[1]), u = toNum(r[2]), s = toNum(r[3]), a = toNum(r[4])

    if (path.includes('/dijual/')) { segmentTotals.dijual.views += v; segmentTotals.dijual.users += u; segmentTotals.dijual.sessions += s; segmentTotals.dijual.aetSum += a; segmentTotals.dijual.count++ }
    else if (path.includes('/disewa/')) { segmentTotals.disewa.views += v; segmentTotals.disewa.users += u; segmentTotals.disewa.sessions += s; segmentTotals.disewa.aetSum += a; segmentTotals.disewa.count++ }
    else if (path.includes('/articles-all/')) { segmentTotals.blog.views += v; segmentTotals.blog.users += u; segmentTotals.blog.sessions += s; segmentTotals.blog.aetSum += a; segmentTotals.blog.count++ }
  }
  ...
  return {
    ...
    dijual: { views: segmentTotals.dijual.views, users: segmentTotals.dijual.users, sessions: segmentTotals.dijual.sessions, aet_seconds: segmentTotals.dijual.aet_seconds },
    disewa: { views: segmentTotals.disewa.views, users: segmentTotals.disewa.users, sessions: segmentTotals.disewa.sessions, aet_seconds: segmentTotals.disewa.aet_seconds },
    blog: { views: segmentTotals.blog.views, users: segmentTotals.blog.users, sessions: segmentTotals.blog.sessions, aet_seconds: segmentTotals.blog.aet_seconds },
  }
}
...
export function formatFlow2DetectionLabel(result) {
  const month = new Date(result.month.year, result.month.month - 1)
    .toLocaleDateString('en-US', { month: 'short', year: 'numeric' })

  const segLabel = { all_organic: 'All Organic', dijual: '/dijual/', disewa: '/disewa/', blog: 'Blog' }
...
function z() {
  return { views: 0, users: 0, sessions: 0, aetSum: 0, count: 0, aet_seconds: 0 }
}
