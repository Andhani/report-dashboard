...
function sumGA4(slugs, ga4Map) {
  let views = 0, users = 0, sessions = 0, aetSum = 0, aetCount = 0
  for (const slug of slugs) {
    const d = ga4Map[slug] || Z_GA4
    views += d.views
    users += d.users
    sessions += d.sessions
    if (d.aet_seconds > 0) { aetSum += d.aet_seconds; aetCount++ }
  }
  return {
    views,
    users,
...
export function computeBlogLeads(blogUrls, flow1Data, flow2Data, slot) {
  ...
  const updates = published.filter(u => u.content_type === 'Update' || u.content_type === 'Optimize')

  const createTraffic = sumGA4(creates.map(u => u.slug), ga4Map)
  const updateTraffic = sumGA4(updates.map(u => u.slug), ga4Map)
  ...
  return {
    monthLabel: `${getIndonesianMonth(month)} ${year}`,
    creates: { count: creates.length, traffic: createTraffic, estimated: estimated(createTraffic) },
    updates: { count: updates.length, traffic: updateTraffic, estimated: estimated(updateTraffic) },
    grandTotal: {
      count: published.length,
      traffic: sumGA4([...creates, ...updates].map(u => u.slug), ga4Map),
    },
    rates,
    siteWide: { totalViews, totalUsers, totalSessions, clickContact },
  }
...
export function buildBCLeadsCSV(block) {
  ...
  return [
    ...
    [count, 'Sum of Views',         fmtNum(traffic.views),    'Views-based',    fmtEst(estimated.views),    'Total Org Views', fmtNum(siteWide.totalViews)],
    ['',   'Sum of Active Users',   fmtNum(traffic.users),    'Users-based',    fmtEst(estimated.users),    'Click_Contact',   fmtNum(siteWide.clickContact)],
    ['',   'Sum of Sessions',       fmtNum(traffic.sessions), 'Sessions-based', fmtEst(estimated.sessions), 'Rate',            fmtRate(rates.leadPerViews)],
...
export function buildBlogLeadsCSV(block) {
  ...
  return [
    ...
    ['Content Type', 'Jumlah', 'Metric', 'Value', 'Metric', 'Value', 'Metric', 'Value'],
    ['Create',      creates.count,      'Sum Views',   fmtNum(creates.traffic.views),    'Views-based',    fmtEst(creates.estimated.views),    'Total Org Views', fmtNum(siteWide.totalViews)],
    ['Update',      updates.count,      'Sum Users',   fmtNum(creates.traffic.users),    'Users-based',    fmtEst(creates.estimated.users),    'Click_Contact',   fmtNum(siteWide.clickContact)],
    ['Grand Total', grandTotal.count,   'Sum Sessions',fmtNum(creates.traffic.sessions), 'Sessions-based', fmtEst(creates.estimated.sessions), 'Rate',            fmtRate(rates.leadPerViews)],
    ['',            '',                 'Avg AET',     fmtAET(creates.traffic.aet_seconds)],
    [''],
    ['', 'UPDATE CONTENT', '', 'Estimated Leads (Update)', '', 'Lead per Users'],
    ['', updates.count,    'Sum Views',    fmtNum(updates.traffic.views),    'Views-based',    fmtEst(updates.estimated.views),    'Total Org Users',    fmtNum(siteWide.totalUsers)],
    ['', '',               'Sum Users',    fmtNum(updates.traffic.users),    'Users-based',    fmtEst(updates.estimated.users),    'Click_Contact',      fmtNum(siteWide.clickContact)],
    ['', '',               'Sum Sessions', fmtNum(updates.traffic.sessions), 'Sessions-based', fmtEst(updates.estimated.sessions), 'Rate',               fmtRate(rates.leadPerUsers)],
    ['', '',               'Avg AET',      fmtAET(updates.traffic.aet_seconds), '', '', 'Lead per Sessions'],
    ['', '', '', '', '', '', 'Total Org Sessions', fmtNum(siteWide.totalSessions)],
    ['', '', '', '', '', '', 'Click_Contact',      fmtNum(siteWide.clickContact)],
    ['', '', '', '', '', '', 'Rate',               fmtRate(rates.leadPerSessions)],
    [''],
