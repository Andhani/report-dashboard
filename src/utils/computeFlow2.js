...
export const METRICS = [
  ...
  {
    id: "users",
    label: "Total Active Users (GA4)",
    source: "ga4",
    allOnly: false,
  },
  {
    id: "sessions",
    label: "Total Sessions (GA4)",
    source: "ga4",
    allOnly: false,
  },
...
export function buildFlow2CSV(outputData, slots) {
  ...
  const headers = [
    "Metric",
    ...SEGMENTS.flatMap((seg) => slots.map((s) => `${seg.label} ${s.label}`)),
  ];
        ...
        if (metric.allOnly && seg.id !== "all_organic") {
          ...
          cells.push(
            val !== null && val !== undefined
              ? formatMetricValue(metric.id, val)
              : "0",
          );
        }
      }
