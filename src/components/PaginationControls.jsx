...
export default function PaginationControls({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-1 py-2 text-xs text-muted">
      <div className="flex items-center gap-2">
        <span className="text-2xs text-muted">Rows</span>
        <select
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-2xs text-muted px-2 whitespace-nowrap">
            {page} / {pageCount}
          </span>
...
function PageBtn({ label, title, onClick, disabled }) {
  return (
    <button
      ...
      disabled={disabled}
      title={title}
      className="w-7 h-7 flex items-center justify-center rounded-[6px] border border-border text-muted hover:bg-surface-2 hover:text-ink disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
    >
      {label}
    </button>
  );
