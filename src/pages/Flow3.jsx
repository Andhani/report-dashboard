...
export default function Flow3() {
  ...
  if (!flow1Window) {
    return <GatingMessage icon="⚙️" title="Flow 1 window not set" desc="Set the Flow 1 rolling window in Settings first." to="/settings" btnLabel="Go to Settings" />
  }

  if (!hasFlow1 && !hasFlow2) {
    return <GatingMessage icon="📥" title="No data available" desc="Complete Flow 1 and Flow 2 data import before computing leads." to="/flow1" btnLabel="Go to Flow 1" />
  }

  const bcBlock = currentSlot ? computeBCLeads(bcUrls, flow1Data, flow2Data, currentSlot) : null
  const blogBlock = currentSlot ? computeBlogLeads(blogUrls, flow1Data, flow2Data, currentSlot) : null

  function handleDownloadCSV(project) {
    if (!currentSlot) return
    const block = project === 'bc' ? bcBlock : blogBlock
    const rows = project === 'bc' ? buildBCLeadsCSV(block) : buildBlogLeadsCSV(block)
    const label = currentSlot.label.replace(' ', '')
    downloadCSV(rows, `${project === 'bc' ? 'BC' : 'Blog'}_Leads_Summary_${label}.csv`)
  }

  async function handlePushSheets(project) {
    const ssId = extractSpreadsheetId(sheetsUrl)
    if (!ssId) { alert('No spreadsheet URL in Settings.'); return }
  ...
  return (
    <div className="space-y-6">
      ...
      {currentSlot && (
        <div className="card p-4 flex flex-wrap items-center gap-3">
          ...
          {['bc', 'blog'].map(proj => {
            ...
            const err = ps?.startsWith(proj + '_error:') ? ps.slice(proj.length + 7) : null
            return (
              <div key={proj} className="flex items-center gap-2">
                <button onClick={() => handleDownloadCSV(proj)} className="btn-secondary text-xs">
                  ⬇ {proj === 'bc' ? 'BC' : 'Blog'} CSV
                </button>
                <button
      ...
      {!currentSlot && (
        <div className="card p-8 text-center text-gray-400 text-sm">No slots defined in the current window.</div>
      )}
    </div>
  )
}
    ...
    <div className="card p-4 bg-yellow-50 border-yellow-200">
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm text-yellow-900">
          ...
          {!hasFlow1 && <span> <Link to="/flow1" className="underline">Flow 1 data missing</Link>.</span>}
          {!hasFlow2 && <span> <Link to="/flow2" className="underline">Flow 2 data missing</Link>.</span>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="card p-6">
        <div className="text-lg font-semibold text-gray-900 mb-1">{monthLabel}</div>
        <div className="text-xs text-gray-500 mb-5">Data Source = GA4</div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Traffic Summary */}
          <MetricBlock title="Traffic Summary" subtitle={`Bottom Content: ${count} URLs published`}>
            <MetricRow label="Sum of Views"       value={fmtNum(traffic.views)} />
            <MetricRow label="Sum of Active Users" value={fmtNum(traffic.users)} />
            <MetricRow label="Sum of Sessions"    value={fmtNum(traffic.sessions)} />
      ...
      <div className="card p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            <div>
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">CREATE CONTENT ({creates.count})</div>
              <MetricRow label="Sum Views"    value={fmtNum(creates.traffic.views)} />
              <MetricRow label="Sum Users"    value={fmtNum(creates.traffic.users)} />
              <MetricRow label="Sum Sessions" value={fmtNum(creates.traffic.sessions)} />
              <MetricRow label="Avg AET"      value={fmtAET(creates.traffic.aet_seconds)} />
            </div>
            <div>
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">UPDATE CONTENT ({updates.count})</div>
              <MetricRow label="Sum Views"    value={fmtNum(updates.traffic.views)} />
              <MetricRow label="Sum Users"    value={fmtNum(updates.traffic.users)} />
              <MetricRow label="Sum Sessions" value={fmtNum(updates.traffic.sessions)} />
              <MetricRow label="Avg AET"      value={fmtAET(updates.traffic.aet_seconds)} />
            </div>
            <div className="border-t border-gray-200 pt-2">
              <div className="text-xs font-semibold text-gray-500 mb-1">Grand Total: {grandTotal.count} URLs</div>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div>
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Est. Leads (Create)</div>
            ...
            <div>
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Est. Leads (Update)</div>
...
function MetricRow({ label, value, highlight }) {
  return (
    <div className="flex items-center justify-between py-1 border-b border-gray-50">
      ...
      <span className={`text-sm font-medium tabular-nums ${highlight ? 'text-green-700' : 'text-gray-900'}`}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-3">
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-xs text-gray-600"><span>{totalLabel}</span><span className="tabular-nums">{total}</span></div>
        <div className="flex justify-between text-xs text-gray-600"><span>Click_Contact_Agent</span><span className="tabular-nums">{contact}</span></div>
        <div className="flex justify-between text-xs font-semibold text-brand-700 pt-1 border-t border-gray-200"><span>Rate</span><span>{rate}</span></div>
      </div>
    </div>
  )
}
